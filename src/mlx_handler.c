...
/*   Updated: 2023/10/21 15:43:20 by kglebows         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
mlx_image_t	*ft_put_imp(int x, int y, t_map *map)
{
	mlx_texture_t			*png;
	char					*path;
	char					*random_path;
	...
	if (!img)
		return (ft_error(-21), NULL);
	mlx_resize_image(img, TILE_SIZE * 0.75, TILE_SIZE * 0.75);
	if (mlx_image_to_window(map->mlx, img, (x * TILE_SIZE) + TILE_SIZE * 0.125, (y * TILE_SIZE) + TILE_SIZE * 0.3) < 0)
		return (ft_error(-22), NULL);
	map->after_img[x][y] = map->img_map[x][y];
	return (img);
}
